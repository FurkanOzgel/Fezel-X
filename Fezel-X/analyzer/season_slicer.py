...
def slice_seasons(data_list):
    mainData = data_list
    minPrice = [100000000]
    maxPrice = [0]

    for i in mainData:
        if i < float(minPrice[0]):                   # Min ve max noktasını almak için
            minPrice = [float(row[1]["O"]), row[1]["Open_Time"]]
        if float(row[1]["O"]) > float(maxPrice[0]):
            maxPrice = [float(row[1]["O"]), row[1]["Open_Time"]]
...
slice_seasons([0.5183270768489299, 0.35834716234604574, 0.46708185896399523, 0.4594617939415237, 0.5811862604474414, 1.1506225291007945, 0.8579107170646479, 1.1317271940113551, 1.1907992460130674, 1.5312603318479594, 1.4324812908967393, 1.0916864458208118])